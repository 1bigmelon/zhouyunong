...
export default {
  ...
  data() {
    return {
      ...
      form: this.$form.createForm(this, { name: 'edit_category' }),
      rules: {
        id: ['id'],
        name: ['name', { rules: [
          {
  ...
  mounted() {
    Promise.all([
      this.$api.getCategoryInfo(this.$route.params.id),
      this.$api.getAllOrgs(),
      ...
      this.form.setFieldsValue(this.categoryInfo)
  ...
  methods: {
    goBack() {
      ...
      this.$modal.confirm({
        ...
        cancelText: '取消',
        onOk() {
          that.$router.push('/category/manage')
        }
      })
    },
    submitBtnHdl(e) {
      ...
      this.form.validateFields((err, values) => {
        ...
        const newInfo = { id: this.categoryInfo.id }
        let updated = false
        for (const key in this.categoryInfo) {
          if (values[key] === undefined) continue
          if (values[key] !== this.categoryInfo[key]) {
            updated = true
            Object.assign(newInfo, JSON.parse(`{"${categoryNameMap[key]}":"${values[key]}"}`))
          }
        }
        ...
        this.$api.changeCategoryInfo(newInfo)
          .then((res) => {
            if (!res.data.status) {
